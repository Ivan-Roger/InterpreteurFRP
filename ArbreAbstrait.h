...
class NoeudInstLire : public Noeud {
    ...
    //  et son fils : la variable a lire (+ les autres variables a lire)
public:
    ...
    int executer(); // Exécute l'instruction lire : demande a l'utilisateur la valeur de chaque variable;
...
//  qui a autant de fils que de cas plus la variable
  public:
    NoeudInstSelon(Noeud* var, Noeud* defaut=nullptr);
     // Construit une "instruction Selon avec sa variable et son possible cas par defaut
   ~NoeudInstNoeud() {} // A cause du destructeur virtuel de la classe Noeud
    int executer();  // Exécute l'instruction selon : execute la sequence d'instruction pour le cas adapté.
    void ajoute(Noeud* cas);  // Ajoute une variable
    void translate(std::ostream& out, int offset=0); // Traduction en langage compilable
